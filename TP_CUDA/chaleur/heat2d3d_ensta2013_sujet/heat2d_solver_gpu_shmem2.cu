...
runTest(int argc, char** argv) 
{
  ...
  for (real_t t=0.0f; t<TMAX; t+=(2*DT), iTime+=2) {
    ...
    if (useOrder2) {
      ...
      heat2d_ftcs_sharedmem2_order2_kernel<<< grid, threads >>>( d_data1, d_data2, 
      								_pitch1, NX, NY);
      
      // check if kernel execution generated and error
      getLastCudaError("Kernel execution failed");
      
      heat2d_ftcs_sharedmem2_order2_kernel<<< grid, threads >>>( d_data2, d_data1, 
      								_pitch1, NX, NY);
      // check if kernel execution generated and error
      getLastCudaError("Kernel execution failed");
      ...
      heat2d_ftcs_sharedmem2_order4_kernel<<< grid, threads >>>( d_data1, d_data2, 
      								_pitch1, NX, NY);
      
      // check if kernel execution generated and error
      getLastCudaError("Kernel execution failed");
      
      heat2d_ftcs_sharedmem2_order4_kernel<<< grid, threads >>>( d_data2, d_data1, 
      								_pitch1, NX, NY);
      // check if kernel execution generated and error
      getLastCudaError("Kernel execution failed");
    }
