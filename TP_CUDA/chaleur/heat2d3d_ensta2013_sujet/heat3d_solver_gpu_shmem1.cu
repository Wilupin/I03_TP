 ...
 *
 * GPU version : shared memory 1 (use pitched global memory and shared memory)
 *
 * \date 27-dec-2009.
 */
...
runTest(int argc, char** argv) 
{
  ...
  for (real_t t=0.0f; t<TMAX; t+=(2*DT), iTime+=2) {
    ...
    if (useOrder2) {
      ...
      heat3d_ftcs_sharedmem_order2_kernel<<< grid, threads >>>( d_data1, 
								d_data2, 
								_pitch1, 
								NX, NY, NZ);
      ...
      heat3d_ftcs_sharedmem_order2_kernel<<< grid, threads >>>( d_data2, 
								d_data1, 
								_pitch1,
								NX, NY, NZ);
      // check if kernel execution generated and error
      ...
      heat3d_ftcs_sharedmem_order4_kernel<<< grid, threads >>>( d_data1, 
								d_data2, 
								_pitch1, 
								NX, NY, NZ);
      ...
      heat3d_ftcs_sharedmem_order4_kernel<<< grid, threads >>>( d_data2, 
								d_data1, 
								_pitch1,
								NX, NY, NZ);
      // check if kernel execution generated and error
